...
def resolve_prompt(fkey):
    ...
    if callable(raw):
        ...
        if fkey == "directors_cut":
            directors = raw()  # lista di 8 dict {name, brief} random
            ...
            return (
                ...
                f"Other tiles labels exactly: {labels}. "
                ...
                "OUTFIT LOCK: The subject's outfit must remain identical to the ORIGINAL in every tile — same garments, colors, and accessories. Never replace or alter the clothing. "
                "Each tile reshots the subject in a new cinematic environment coherent with that director's world — "
                "change background, setting, lighting, palette, mood, and pose. The subject acts as a protagonist in that director's universe.\n\n"
                f"Director styles:\n{briefs}\n\n"
