        ...
        "prompt": lambda: __import__('random').sample([
            {
                "name": "Hitchcock",
                "brief": (
                    "subject standing in a wide shadowy interior: dark wood paneling, venetian blind shadows "
                    "casting graphic horizontal stripes across the walls and floor. Full body visible in frame. "
                    "Cold desaturated palette: muted grays, sickly greens, deep blacks. "
                    "Hard single-source key light from the side, long dramatic shadows on the floor. "
...
def resolve_prompt(fkey):
    ...
    if callable(raw):
        ...
        if fkey == "directors_cut":
            ...
            labels = ", ".join(d["name"] for d in directors)
            ...
            return (
                ...
                "LABELS: Each tile has a label in italic, Title Case, centered at the bottom of its own individual tile, "
                "very small font size relative to the single tile (not the full image) — elegant light-weight sans-serif, "
                "white text at 85% opacity with a subtle dark drop-shadow. No label at top. "
                "Center tile label: Original. "
                f"Other 8 tiles labels with proper capitalization: {labels}. "
                "Do NOT use any other director name. Do NOT invent or substitute names. "
                "No other text, logos, subtitles, or watermarks anywhere. Keep the 3x3 alignment perfectly straight and clean.\n\n"
                ...
                "POSE: The subject must hold the exact same pose and framing (full body or same crop) across all 8 tiles. Do not change the pose between tiles. "
                ...
                "What changes per tile: ONLY the background environment, setting, lighting, color palette, and mood. "
                "CRITICAL — INTEGRATION: Do NOT composite or paste the subject onto a background. "
                "Fully recreate the subject from scratch in each environment — relight the skin, cast real shadows, "
                "reflect ambient colors onto the outfit, match the depth of field. "
                "The subject must feel physically present in the scene, not superimposed.\n\n"
                f"Director styles:\n{briefs}\n\n"
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            contents=[
                ...
                "2. ACCESSORIES: all visible jewelry, shoes, belts, hats, gloves, bags\n"
                ...
                "Focus exclusively on what they wear. "
...
def _run_generation(chat_id, uid, username, data):
    ...
    # ── DIRECTORS CUT: pipeline a due fasi ──────────────────────────────
    if fkey == "directors_cut":
        try:
            # PHASE 1 — estrazione outfit
            ...
            # PHASE 2 — generazione collage con master face + outfit testuale
            bot.send_message(chat_id, "🎬 <i>Phase 2 — Generazione collage in corso...</i>", parse_mode="HTML")

            # Inietta descrizione outfit nel prompt
            outfit_block = (
                f"OUTFIT REFERENCE (extracted from source photo — recreate exactly in every tile):\n{outfit_desc}\n\n"
            )
            ...
            # NON includere img_bytes originale — generazione da zero
            response = client.models.generate_content(
                model=MODEL_ID,
                contents=contents,
                config=genai_types.GenerateContentConfig(
