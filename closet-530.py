...
# --- BLOCCHI VALERIA CROSS — STILE GEMINI SECTIONED ---
CLOSET_HEADER = (
...
CLOSET_SUBJECT = (
    ...
    "Completely hairless body (arms, legs, underarms, chest — no body hair at all, body hair NO!). "
    "Smooth porcelain skin on ALL body surfaces. "
    "Sculptural hourglass silhouette — defined waist, wide hips, full rounded figure. "
...
CLOSET_TECH = (
    ...
    "8K, cinematic, 85mm, f/2.8, natural bokeh. "
    "Full photographic coherence: unified exposure, unified color grade, unified depth of field.\n\n"
    "**Watermark:** 'feat. Valeria Cross 👠' — elegant champagne cursive, very small, bottom center/left, 90% opacity.\n"
)
...
CLOSET_NEG = (
    ...
    "**NEGATIVE PROMPT — BODY:** male flat chest, masculine frame, blurry hands, extra fingers, mismatched skin tone, disconnected face, faceswap artifacts.\n"
    "**NEGATIVE PROMPT — GEMINI SAFETY:** JSON output, text output, captions, metadata blocks, structured data. IMAGE GENERATION ONLY. NO JSON LEAKAGE.\n"
)
...
def handle_input(m):
    ...
    final_p = (
        ...
        f"{CLOSET_SUBJECT}\n"
        f"**Outfit:** Extract outfit, accessories, shoes and environment from the scene description above. "
        f"Generate a COMPLETELY NEW editorial photograph — new pose, new angle, new framing. NOT a copy.\n\n"
        f"{CLOSET_FACE}\n"
        f"{CLOSET_HAIR}\n"
        f"{CLOSET_TECH}\n"
        f"{CLOSET_NEG}"
