# ---------------------------------------------------------
# VERSION: V2.29 (Cohesive Integration - Anti-Faceswap)
# TIMESTAMP: 2026-02-14 23:35 WET
# CHANGELOG:
# - FIX V2.28 BUG: Addressed the "bad faceswap" issue where the model was
#   copy-pasting the face onto the reference body instead of regenerating.
# - NEW INSTRUCTION: Added strict commands for lighting cohesion and
#   explicitly forbidding "cut-and-paste" looks.
# - RETAINED: Smart Context (Temu vs Reality) logic from V2.28.
# ---------------------------------------------------------
...
def show_settings(m):
    bot.send_message(m.chat.id, "<b>👗 Valeria Closet V2.29 (Anti-Faceswap)</b>\nScegli il formato e il numero di scatti:", 
...
def analyze_outfit_vision(img_bytes):
    try:
        # V2.28 Logic: Analyze Outfit AND Background
        prompt = """
        Analyze this image in 2 parts.
        1. OUTFIT: Describe materials, cut, colors, textures using technical fashion terms.
        2. BACKGROUND/CONTEXT: Is it a plain studio/catalog background OR a specific real-world location (e.g., snow, beach, street, forest)?
        If it is a specific location, describe it in detail. If it is plain/neutral, just say "Plain Studio".
        Max 60 words total.
        """
...
def generate_closet_task(img_bytes, ar_scelto, user_notes, vision_desc, edit_mode=False):
    try:
        ...
        IDENTITY & COHESION (Cruciale)
        Usa l'immagine master_face.png come riferimento assoluto per i tratti somatici del volto.
        IMPORTANTE: Il volto deve essere perfettamente integrato con l'illuminazione e l'atmosfera della scena generata. Non deve MAI sembrare un "faceswap" o un incollaggio. La pelle, le ombre e i riflessi sul viso devono corrispondere esattamente al resto del corpo e all'ambiente circostante. Il collo e l'attaccatura devono essere naturali.

        Regola OUTFIT & AMBIENTAZIONE (Smart Context V2.28)
        Analisi Immagine: {safe_outfit}
        
        LOGICA AMBIENTAZIONE:
        1. SE l'analisi rileva un LUOGO SPECIFICO: MANTIENI quell'ambientazione, ricreando l'atmosfera e la luce descritte.
        2. SE l'analisi dice "Studio" o "Plain": INVENTA un'ambientazione di lusso coerente con l'abito.
        
        L’outfit deve essere riprodotto con massima fedeltà.
        La posa DEVE essere nuova, dinamica ed editoriale.
        ...
        female face, woman, girl, young, unrealistic skin, long feminine hair, body hair, chest hair, breast hair, peli sul seno, static pose, mannequin pose, watermark invasivo, bad faceswap, pasted face, unnatural neck.
        """
...
def handle_reply_edit(m):
    ...
    if file_id:
        bot.reply_to(m, "🔄 Elaborazione modifica (V2.29)...")
...
@app.route('/')
def h(): return "Bot Online - V2.29 (Cohesive)"
