# ---------------------------------------------------------
# VERSION: V2.24 (Gemini 2.0 Pro Experimental)
# TIMESTAMP: 2026-02-13 23:10 WET
# CHANGELOG:
# - Switched GEN_MODEL to 'models/gemini-2.0-pro-exp-02-05'.
# - This model typically supports Image-to-Image (unlike Imagen 3).
# - Added Diagnostic print on startup to verify visible models.
# - Maintained V2.20 logic (Pose Unlock).
# ---------------------------------------------------------
...
# --- MODELLI ---
# Vision per analisi rapida
VISION_MODEL = "models/gemini-2.0-flash"
# TENTATIVO 4: Il modello Pro Experimental di Febbraio 2025
GEN_MODEL = "models/gemini-2.0-pro-exp-02-05"
...
def check_available_models():
    try:
        # Questo serve a noi per capire cosa vede il tuo account se crasha ancora
        logger.info("--- LISTA MODELLI DISPONIBILI ---")
        for m in client.models.list(config={"page_size": 100}):
            if "generateContent" in m.supported_actions:
                logger.info(f"Model: {m.name} | Display: {m.display_name}")
        logger.info("-------------------------------")
    except Exception as e:
        logger.error(f"Errore check modelli: {e}")
...
def show_settings(m):
    bot.send_message(m.chat.id, f"<b>👗 Valeria Closet Settings (V2.24)</b>\nEngine: {GEN_MODEL}\nScegli il formato e il numero di scatti:", 
...
def handle_reply_edit(m):
    ...
    if file_id:
        bot.reply_to(m, "🔄 Elaborazione modifica (V2.24)...")
...
if __name__ == "__main__":
    check_available_models() # Diagnostica all'avvio
    app = flask.Flask(__name__)
    @app.route('/')
    def h(): return "Bot Online - V2.24"
