...
def _send_confirmation(chat_id, uid, fname, reply_to=None):
    ...
    text = f"🎨 Filtro: <b>{fname}</b>\n\nProcedere con la generazione?"
    if reply_to:
        bot.send_message(chat_id, text, reply_to_message_id=reply_to, reply_markup=markup)
    else:
...
def _run_generation(chat_id, uid, username, data):
    ...
    try:
        ...
        contents = [filter_prompt, img_part]
