...
def cmd_done(m):
    ...
    if uid not in mosaic_collecting:
        ...
        return
    _finalize_mosaic(uid, m.chat.id)
...
def _finalize_mosaic(uid, chat_id):
    ...
    if n == 4:
        ...
        needed = 4 if n < 4 else (9 if n < 9 else None)
        if needed:
            missing = needed - n
            bot.send_message(chat_id,
                f"❌ Hai inviato <b>{n} foto</b>.\n"
                f"Ne servono ancora <b>{missing}</b> per il mosaico {int(needed**0.5)}×{int(needed**0.5)}.\n"
                f"Continua a inviare foto o usa /mosaic per ricominciare.")
        else:
            bot.send_message(chat_id,
                f"❌ Hai inviato <b>{n} foto</b> — troppo per 3×3 (max 9), non abbastanza per 4×4.\n"
                f"Usa /mosaic per ricominciare con 4 o 9 foto.")
        # Rimetti le foto in sessione così non le perde
        mosaic_collecting[uid] = {'photos': photos, 'timer': None}
        return

    bot.send_message(chat_id, f"🖼️ Assemblando mosaico {cols}×{rows}...")
    ...
    def build():
        try:
            ...
            # Dimensione tile = minima larghezza e altezza tra tutte le foto
            min_w = min(img.width for img in images)
            min_h = min(img.height for img in images)

            def crop_center(img, tw, th):
                # Resize mantenendo proporzioni, poi crop centrato
            ...
            tiles = [crop_center(img, min_w, min_h) for img in images]

            grid = Image.new("RGB", (min_w * cols, min_h * rows), (0, 0, 0))
            for idx, tile in enumerate(tiles):
                col = idx % cols
                row = idx // cols
                grid.paste(tile, (col * min_w, row * min_h))
            ...
            bot.send_document(
                chat_id,
                out,
                visible_file_name=f"mosaic_{cols}x{rows}.jpg",
                caption=f"✅ Mosaico {cols}×{rows} — {n} foto"
            )
            logger.info(f"   ✅ Mosaic {cols}x{rows} inviato a uid={uid}")
        except Exception as e:
...
def handle_photo(m):
    ...
    if uid in mosaic_collecting:
        ...
        # Cancella timer inattività precedente
        ...
        if n in (4, 9):
            bot.reply_to(m, f"✅ {n} foto ricevute — pronto! Invia /done per assemblare o continua ad aggiungere.")
        elif n > 9:
            bot.reply_to(m, f"⚠️ Hai già {n} foto — il massimo è 9 (3×3). Invia /done per assemblare o /mosaic per ricominciare.")
        else:
            needed_next = 4 if n < 4 else 9
            bot.reply_to(m, f"📷 Foto {n} ricevuta. Ne servono ancora <b>{needed_next - n}</b> per il prossimo formato.")

        # Timer 5 minuti inattività
