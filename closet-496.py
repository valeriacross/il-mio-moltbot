...
def handle_input(m):
    ...
    # Mostra solo la scena leggibile, non i blocchi tecnici interni
    scene_preview = user_instruction
    if len(scene_preview) > 3800:
        scene_preview = scene_preview[:3800] + "\n<i>... (troncato)</i>"
    bot.send_message(m.chat.id,
        f"{header}📝 <b>Scena:</b>\n<code>{html.escape(scene_preview)}</code>\n\nProcedere?",
