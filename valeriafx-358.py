...
def _run_generation(chat_id, uid, username, data):
    ...
    if fkey == "remix_mosaic":
        ...
        logger.info(f"   🎲 Remix Mosaic — filtri: {selected}")
        bot.send_message(chat_id,
            f"🎲 Generando 8 tile con filtri random...\n"
            f"⏳ Può richiedere fino a 60 secondi. Attendi.")

        tile_results = [None] * 8

        def generate_tile(idx, fk):
            fp = resolve_prompt(fk)
                ...
                if response.candidates and response.candidates[0].finish_reason == "STOP":
                    for part in response.candidates[0].content.parts:
                        if part.inline_data:
                            ...
                            logger.info(f"   ✅ Tile {idx+1}/8 ({fk}) OK")
                            return
                logger.warning(f"   ⚠️ Tile {idx+1}/8 ({fk}) fallita")
            except Exception as e:
                logger.error(f"   ❌ Tile {idx+1}/8 ({fk}) errore: {e}")
