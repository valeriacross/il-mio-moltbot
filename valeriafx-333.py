        ...
        "prompt": lambda: __import__('random').sample([
            {
                "name": "Hitchcock",
                "brief": (
                    "subject framed voyeuristically in a shadowy interior: dark wood paneling, venetian blind shadows "
                    "casting graphic stripes across the scene. Cold desaturated palette: muted grays, sickly greens, deep blacks. "
                    "Hard single-source key light sculpting the face from one side. Suspenseful stillness, psychological tension in the pose. 35mm noir sharpness."
                )
            },
            {
                "name": "Nolan",
    ...
    if callable(raw):
        ...
        if fkey == "directors_cut":
            ...
            return (
                ...
                "LABELS: Each tile has a label in italic lowercase, centered at the bottom of the tile, very small font size (max 2% of tile height), "
                "white text at 90% opacity with a thin dark drop-shadow for legibility. No label at top. "
                "Center tile label: original. "
                f"Other 8 tiles labels, exactly these names in this order: {labels}. "
                ...
                "What changes per tile: background environment, setting, lighting, color palette, mood, and pose. "
