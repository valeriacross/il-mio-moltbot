...
# --- ESTRAZIONE OUTFIT PER DIRECTORS CUT ---
def extract_outfit_description(img_bytes):
    """Phase 1: estrae descrizione testuale dell'outfit dalla foto. Solo vestiti, niente volto/corpo."""
    try:
        ...
        response = client.models.generate_content(
            ...
            contents=[
                img_part,
                "Analyze this image and describe ONLY the outfit in precise detail:\n"
                "1. GARMENTS: every piece of clothing (name, color, fabric, cut, fit, length, details, embellishments, closures, pattern)\n"
                "2. ACCESSORIES: all visible jewelry, shoes, belts, hats, gloves, bags, wings, props\n"
                "3. OVERALL STYLE: fashion aesthetic, era, mood of the look\n"
                "Be extremely detailed and precise. "
                "Do NOT describe the person's face, hair, body, age, gender, background or lighting. "
                "Focus exclusively on what they wear and carry. "
                "Write in plain English, no bullet points, no headers, continuous prose."
            ],
        ...
        if response.text:
            ...
            logger.info(f"   👗 Outfit estratto ({len(desc)} chars)")
            return desc
        return None
    except Exception as e:
        logger.warning(f"   ⚠️ Estrazione outfit fallita: {e}")
        return None

# ============================================================
# LABEL OVERLAY — DIRECTORS CUT
# ============================================================
def add_directors_labels(img_bytes, directors):
    """
    Aggiunge label minimali ai 9 tile del Directors Cut via Pillow.
    Layout 3x3: tile (col, row) = posizione nel grid.
    directors: lista di 8 dict {name} in ordine top-left → bottom-right, centro escluso.
    """
    try:
        ...
        # Font — usa default se DejaVu non disponibile
        font_size = max(24, tile_h // 16)
        try:
        ...
        # Posizioni tile in ordine row-major, centro (1,1) = "Original"
        positions = [
            (0,0),(1,0),(2,0),
            (0,1),      (2,1),
            (0,2),(1,2),(2,2),
        ]
        labels = ["Original"] + [d["name"] for d in directors]
        # Riordina: centro è posizione 4 (indice 3 nella lista completa)
        all_labels = [None] * 9
        all_labels[4] = "Original"
        director_names = [d["name"] for d in directors]
        idx = 0
        for i in range(9):
            if i == 4:
                continue
            all_labels[i] = director_names[idx]
            idx += 1
        ...
        for i, label in enumerate(all_labels):
            ...
            # Centra il testo in basso nel tile
            bbox = draw.textbbox((0, 0), label, font=font)
            text_w = bbox[2] - bbox[0]
            text_h = bbox[3] - bbox[1]
            x = tile_x + (tile_w - text_w) // 2
            y = tile_y + tile_h - text_h - int(tile_h * 0.04)

            # Drop shadow
            shadow_offset = max(1, font_size // 18)
            draw.text((x + shadow_offset, y + shadow_offset), label, font=font,
                      fill=(0, 0, 0, 160))
            # Testo bianco 85% opacità
            draw.text((x, y), label, font=font, fill=(255, 255, 255, 217))
        ...
        logger.error(f"   ❌ add_directors_labels fallita: {e}")
        return img_bytes  # fallback: immagine originale senza label
        ...
        "prompt": lambda: __import__('random').sample([
            {
                "name": "Hitchcock",
                "brief": (
                    "BLACK AND WHITE ONLY — render the entire tile in monochrome, no color anywhere. "
                    "Subject standing full body in a wide shadowy interior: dark wood paneling, venetian blind shadows "
                    "casting graphic horizontal stripes across the walls, floor and the subject's body. "
                    "Hard single-source key light from the side, long dramatic shadows stretching on the floor. "
                    "High contrast monochrome — deep blacks, bright whites, no mid-gray wash. "
                    "Suspenseful stillness, voyeuristic wide-angle framing, 35mm noir sharpness."
                )
            },
            {
                "name": "Nolan",
                "brief": (
                    "subject in a vast architectural space: brutalist concrete corridor, IMAX-scale depth, extreme symmetrical perspective vanishing to infinity. "
                    "Desaturated steel-blue and charcoal palette with cold practical lights. Dramatic top-down or low-angle lens. "
                    "Time-bending tension in the pose, hyper-real skin texture, IMAX 70mm sharpness and depth."
                )
            },
            {
                "name": "Wes Anderson",
                "brief": (
                    "subject centered in a perfectly symmetrical pastel-colored room: mint walls, vintage furniture, geometric wallpaper, hand-lettered signs. "
                    "Warm golden-hour light, flat frontal framing, deadpan direct gaze at camera. "
                    "Palette: dusty rose, ochre, powder blue, ivory. Whimsical retro props, immaculate composition, storybook stillness."
                )
            },
            {
                "name": "Leone",
                "brief": (
                    "subject placed in a sunbaked desert landscape or dusty frontier town: cracked earth, wooden saloon facades, golden hour haze. "
                    "Extreme contrast: blinding highlights, deep shadow pockets. Palette: scorched golds, rust oranges, sepia browns. "
                    "Operatic tension, sweat and dust on every surface, extreme wide angle emphasizing the vast empty setting."
                )
            },
            {
                "name": "Spielberg",
                "brief": (
                    "subject in a warm suburban or adventure setting: golden warm light streaming through a doorway or forest canopy, "
                    "lens flares punctuating the scene. Palette: warm ambers, soft greens, hopeful blues. "
                    "Low camera angle looking up at subject with wonder. Cinematic optimism, emotional openness, Kodak-film warmth."
                )
            },
            {
                "name": "Kubrick",
                "brief": (
                    "subject placed in a hypnotic one-point perspective environment: endless symmetrical corridor, grand ballroom, or brutalist interior. "
                    "Cold clinical palette: stark whites, deep reds, institutional greens. "
                    "Perfect frontal symmetry, subject dead-center, unsettling stillness, wide-angle slight distortion. "
                    "Harsh even fluorescent or practical lighting. Dehumanizing grandeur."
                )
            },
            {
                "name": "Tarantino",
                "brief": (
                    "subject in a hyper-stylized retro interior: diner booth, motel room, or warehouse with bold graphic colors. "
                    "Palette: saturated primary reds, yellows, warm wood tones, deep shadows. "
                    "Low-angle shot from below looking up, dynamic tension in the pose, exploitation-cinema grain. "
                    "Sharp 35mm analog look, high contrast, pop-culture cool attitude."
                )
            },
            {
                "name": "Tim Burton",
                "brief": (
                    "subject in a gothic fantasy environment: twisted bare trees, moonlit fog, ornate Victorian architecture, cobblestones. "
                    "Desaturated near-monochrome palette with isolated pops of sickly green or deep crimson. "
                    "Low-key theatrical lighting from below or from a single candle source. "
                    "Expressionist shadows, melancholic atmosphere, dark fairy-tale poetry."
                )
            },
            {
                "name": "Kurosawa",
                "brief": (
                    "subject in an epic natural or feudal landscape: wind-swept plains, bamboo forest, or sparse wooden architecture in rain. "
                    "Earthy desaturated palette: browns, moss greens, slate grays, punchy blacks. "
                    "Weather as visual design — rain, wind, blowing dust. Deep focus, dramatic raking natural light, strong tonal separation. "
                    "Heroic or contemplative pose, gritty organic grain."
                )
            },
            {
                "name": "Coppola",
                "brief": (
                    "subject in a rich, shadowy interior suggesting power and family: mahogany and leather, low amber practical lamps, heavy curtains filtering daylight. "
                    "Palette: deep warm browns, burnished golds, olive shadows. Gordon Willis-style extreme low-key lighting, "
                    "upper half of face often in shadow. Operatic gravitas, stillness charged with tension."
                )
            },
            {
                "name": "Ridley Scott",
                "brief": (
                    "subject in a vast cinematic environment: ancient desert ruins, rain-soaked neon-lit street, or sweeping epic landscape. "
                    "Palette shifts by scene — dusty ochres for epics, cold blues and neon reflections for neo-noir. "
                    "Layers of atmospheric haze, god-rays of light cutting through smoke or mist. "
                    "Epic production design dwarfing the subject, ultra-detailed textures, anamorphic lens flares."
                )
            },
            {
                "name": "Cameron",
                "brief": (
                    "subject in a luminous, immersive environment: bioluminescent jungle, deep ocean interior, or high-tech futuristic base. "
                    "Rich saturated palette: deep teals, electric blues, warm bioluminescent greens and magentas. "
                    "Dramatic motivated lighting from environmental sources — glowing flora, underwater shafts of light. "
                    "Technical precision, heroic framing, cinematic spectacle at massive scale."
                )
            },
            {
                "name": "Scorsese",
                "brief": (
                    "subject in an urban environment charged with energy: jazz club, city street at night, dimly lit bar, or baroque interior. "
                    "Palette: deep blacks, amber and red practicals, neon bleeds. High contrast, rich shadows. "
                    "Dynamic handheld energy or slow-zoom intensity. Saturated film stock feel, urban grit and elegance coexisting."
                )
            },
            {
                "name": "Antonioni",
                "brief": (
                    "subject in a stark modernist or post-industrial landscape: empty piazza, geometric architecture, desolate coastal scene. "
                    "Palette: cold whites, pale grays, faded pastels, alienating negative space. "
                    "Flat diffused overcast light, subject small within the frame or isolated against a graphic background. "
                    "Existential stillness, emotional detachment, silence as visual weight."
                )
            },
            {
                "name": "Lynch",
                "brief": (
                    "subject in a surreal, unsettling interior: red velvet curtains, black-and-white chevron floor, single bare bulb above. "
                    "Palette: deep crimson, stark black, sickly yellows, pools of shadow. "
                    "Extreme chiaroscuro, dreamlike distortion at the edges, practical light sources flickering. "
                    "Uncanny stillness, subject caught between two states of reality."
                )
            },
            {
                "name": "Villeneuve",
                "brief": (
                    "subject in a monumental minimalist environment: vast desert dunes, brutalist concrete exterior, alien landscape at dusk. "
                    "Palette: muted taupes, cold grays, deep orange horizon, sparse cool blues. "
                    "Enormous negative space dwarfing the subject, Deakins-quality natural or single-source lighting. "
                    "Philosophical weight, contemplative stillness, ultra-clean 65mm depth."
                )
            },
            {
                "name": "Aronofsky",
                "brief": (
                    "subject in a claustrophobic, psychologically charged space: mirror-lined room, bare rehearsal studio, crumbling apartment interior. "
                    "Palette: cold clinical whites colliding with deep blacks, flashes of red or sickly green. "
                    "Handheld camera tight on the subject, extreme close-up on face or hands. "
                    "Raw visceral intensity, hyper-real skin texture, obsessive psychological pressure visible in pose and expression."
                )
            },
        ], 8)
    },
...
def paste_original_center(grid_bytes, original_bytes):
    """Incolla la foto originale ridimensionata nel tile centrale (row 2, col 2) del 3x3."""
    try:
        grid = Image.open(io.BytesIO(grid_bytes)).convert("RGB")
        original = Image.open(io.BytesIO(original_bytes)).convert("RGB")
        gw, gh = grid.size
        tile_w, tile_h = gw // 3, gh // 3

        # Ridimensiona originale per riempire il tile (crop centrato)
        orig_ratio = original.width / original.height
        tile_ratio = tile_w / tile_h
        if orig_ratio > tile_ratio:
            new_h = tile_h
        ...
        left = (new_w - tile_w) // 2
        top = (new_h - tile_h) // 2
        original = original.crop((left, top, left + tile_w, top + tile_h))

        # Incolla al centro (col 1, row 1 = offset tile_w, tile_h)
        grid.paste(original, (tile_w, tile_h))

        out = io.BytesIO()
        grid.save(out, format="JPEG", quality=95)
        return out.getvalue()
    except Exception as e:
        logger.error(f"   ❌ paste_original_center fallita: {e}")
        return grid_bytes
    """Risolve il prompt del filtro. Se è callable (lambda), lo esegue e costruisce il testo finale.
    Per directors_cut restituisce (prompt_str, directors_list), altrimenti (prompt_str, None)."""
    raw = FILTERS[fkey]["prompt"]
    if callable(raw):
        if fkey == "fullbody_3x3":
            poses = raw()
            pose_str = ", ".join(f"{i+1}-{p}" for i, p in enumerate(poses))
            return FULLBODY_3X3_BASE.format(poses=pose_str), None
        if fkey == "directors_cut":
            directors = raw()  # 8 registi casuali dal pool
            briefs = "\n".join(f"{d['name']} — {d['brief']}" for d in directors)
            prompt = (
                "Create a single 3x3 grid image (square, 2048x2048, high detail). "
                "CRITICAL — CENTER TILE: The center tile (row 2, col 2) is the ORIGINAL. "
                "Copy it pixel-perfectly from the uploaded reference image. "
                "Do NOT reinterpret, repaint, relight, recolor, crop, reframe, stylize, sharpen, blur, or transform it in any way whatsoever. "
                "It must appear absolutely identical to the uploaded photo. This is non-negotiable.\n\n"
                "Grid rules: 9 equal tiles in a clean 3x3 layout, thin uniform gutters between tiles. "
                "FRAMING: Every tile must show the subject at the same framing as the ORIGINAL — full body or 3/4 body. "
                "Do NOT crop tighter than the original. Do NOT zoom in on the face. Maintain consistent framing across all tiles. "
                "No text, labels, logos, subtitles, or watermarks anywhere in the image. Keep the 3x3 alignment perfectly straight and clean.\n\n"
                "SUBJECT CONSISTENCY (applies to ALL 8 non-ORIGINAL tiles): "
                "The subject's face, facial structure, skin tone, age, hair color and style, facial hair, glasses, "
                "and body proportions must be 100% identical to the uploaded photo in every single tile. "
                "Do not alter, idealize, rejuvenate, or reinterpret the face in any way. "
                "OUTFIT LOCK: The subject's clothing must remain identical to the uploaded photo in every tile — "
                "same garments, colors, cut, and accessories. Never replace or alter the clothing under any circumstance. "
                "POSE: Each tile must show the subject in a different pose and camera angle — vary the framing, body position, and point of view freely across all 8 tiles. No two tiles should share the same pose.\n\n"
                "CRITICAL — CINEMATIC INTEGRATION: Do NOT composite or paste the subject onto a background. "
                "Fully recreate the subject from scratch in each environment. "
                "The director's lighting and color palette must physically affect the subject — "
                "cast real shadows on the body, reflect ambient colors onto the outfit and skin, match depth of field to the scene. "
                "The subject must feel like a protagonist who has always existed in that director's world.\n\n"
                "COLOR GRADING NOTE: If a director's style specifies monochrome or black and white, "
                "the entire tile including the subject and outfit must be rendered in that color mode. No exceptions.\n\n"
                f"Director styles:\n{briefs}\n\n"
                "Output: a single final 3x3 grid image only. No text outside the tile labels."
            )
            return prompt, directors
        return raw(), None
    return raw, None
...
def _send_confirmation(chat_id, uid, fname, reply_to=None):
    try:
        fkey = user_filter[uid]
        filter_prompt, _ = resolve_prompt(fkey) if fkey else ("", None)
        ...
        # Directors Cut: prompt troppo lungo e complesso — mostra solo sommario
        if fkey == "directors_cut":
            text = (
                f"🎬 Filtro: <b>{fname}</b>\n\n"
                f"Pipeline a 3 fasi:\n"
                f"• Phase 1 — analisi outfit\n"
                f"• Phase 2 — generazione 8 tile con master face\n"
                f"• Phase 3 — compositing: originale al centro + label\n\n"
                f"Procedere?"
            )
            if reply_to:
        ...
        # Preview prompt con chunking per gli altri filtri
        CHUNK = 3800
        ...
        logger.error(f"   ❌ _send_confirmation crash: {e}", exc_info=True)
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("🚀 CONFERMA", callback_data="confirm_fx"),
            types.InlineKeyboardButton("❌ ANNULLA", callback_data="cancel_fx")
...
def _run_generation(chat_id, uid, username, data):
    fkey = data['filter_key']
    filter_prompt, directors = resolve_prompt(fkey)
    img_bytes = data['img']
    fname = FILTERS[fkey]["label"]
    ...
    # ── DIRECTORS CUT: pipeline a tre fasi ─────────────────────────────
    if fkey == "directors_cut":
        try:
            # PHASE 1 — estrazione outfit testuale
            bot.send_message(chat_id, "🎬 <i>Phase 1 — Analisi outfit in corso...</i>", parse_mode="HTML")
            outfit_desc = extract_outfit_description(img_bytes)
            if not outfit_desc:
                outfit_desc = "elegant outfit as visible in the reference photo"
                logger.warning("   ⚠️ Outfit non estratto — uso fallback generico")
            else:
                logger.info("   ✅ Phase 1 completata")

            # PHASE 2 — generazione 8 tile con master face + outfit testuale
            # Il centro lo incolleremo noi con Pillow — diciamo a Gemini di lasciarlo neutro
            bot.send_message(chat_id, "🎬 <i>Phase 2 — Generazione tiles in corso...</i>", parse_mode="HTML")

            outfit_block = (
                "OUTFIT REFERENCE — recreate exactly in every tile:\n"
                f"{outfit_desc}\n\n"
            )
            full_prompt = outfit_block + filter_prompt

            contents = [full_prompt]
            if MASTER_PART:
                contents.append(MASTER_PART)
                logger.info("   ✅ MASTER_PART incluso (Directors Cut Phase 2)")
            else:
                logger.warning("   ⚠️ MASTER_PART non disponibile")
            ...
            if not response.candidates:
                bot.send_message(chat_id, "❌ Directors Cut: nessun candidato restituito.")
                return

            candidate = response.candidates[0]
            if candidate.finish_reason != "STOP":
                bot.send_message(chat_id,
                    f"🛡️ Directors Cut bloccato.\nMotivo: <b>{candidate.finish_reason}</b>")
                return
            ...
            for part in candidate.content.parts:
                if part.inline_data:
                    ...
                    # PHASE 3 — Pillow: incolla originale al centro + aggiungi label
                    bot.send_message(chat_id, "🎬 <i>Phase 3 — Compositing finale...</i>", parse_mode="HTML")
                    output_bytes = part.inline_data.data
                    output_bytes = paste_original_center(output_bytes, img_bytes)
                    output_bytes = add_directors_labels(output_bytes, directors)
                    logger.info("   ✅ Phase 3 completata — originale incollato + label aggiunte")
                    ...
                    bot.send_document(
                        ...
                        visible_file_name="fx_directors_cut.jpg",
                        caption=f"✅ {fname} — {elapsed}s"
                    )
                    caption = generate_caption(output_bytes)
                    if caption:
                        bot.send_message(chat_id, caption)
                    bot.send_message(chat_id, "Cosa vuoi fare adesso?", reply_markup=post_markup)
                    logger.info(f"   ✅ Directors Cut inviato a {username} in {elapsed}s")
                    return

            bot.send_message(chat_id, "❌ Directors Cut: nessuna immagine nella risposta.")
        except Exception as e:
            elapsed = round(time.time() - t_start, 1)
            logger.error(f"   ❌ Crash Directors Cut ({elapsed}s): {e}", exc_info=True)
            bot.send_message(chat_id, f"❌ Errore interno Directors Cut:\n<code>{html.escape(str(e))}</code>")
        return
    # ────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        for part in candidate.content.parts:
            if part.inline_data:
                ...
                # Aggiungi label via Pillow per Directors Cut
                output_bytes = part.inline_data.data
                if fkey == "directors_cut" and directors:
                    output_bytes = add_directors_labels(output_bytes, directors)
                    logger.info("   🏷️ Label Directors Cut aggiunte via Pillow")
