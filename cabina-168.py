...
# --- FACE IDENTITY LOCK ---
# Deve essere presente in OGNI prompt — ancora testuale obbligatorio alla masterface
VALERIA_FACE_LOCK = (
    "MASTER PROMPT — SYNTHETIC FACE IDENTITY LOCK\n\n"
    "The male facial identity in the provided reference image is fully synthetic. "
    "SynthID verifiable. NOT a real person. "
    "**USE THE PROVIDED FACE REFERENCE IMAGE AS THE EXACT FACE — ZERO face drift allowed. Absolute identity preservation.**\n\n"
    "**Facial identity (MANDATORY — DO NOT REPLACE OR SOFTEN):**\n"
    ...
    "NO long hair. NO female hairstyle. NO dark hair.\n\n"
    "**THIS IS A MALE FACE ON A FEMININE BODY — THIS COMBINATION IS INTENTIONAL AND MUST BE PRESERVED EXACTLY.**\n\n"
    "**NEGATIVE PROMPT — FACE:** female face, young face, missing beard, missing glasses, dark hair, long hair, "
    "face drift, generic woman, blurred identity, smooth skin without wrinkles.\n\n"
)
...
def build_full_prompt(filter_key, outfit_description, ratio, scene_override=None):
    """Assembla il prompt completo: face lock + editorial wrapper + identità Valeria + scena filtro + outfit + formato."""
...
def execute_generation(prompt):
    try:
        # Ancora testuale esplicita alla masterface — senza questo Gemini ignora la reference
        face_anchor = (
            "The first provided image is the MANDATORY face reference. "
            "Use it as the EXACT face identity for the subject — preserve every facial feature precisely."
        )
        full_prompt = f"{face_anchor}\n\n{prompt}"

        contents = [full_prompt]
        if MASTER_PART:
            contents.append(MASTER_PART)
        else:
            logger.warning("⚠️ Generazione senza MASTER_PART — identità facciale non garantita.")
