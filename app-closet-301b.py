...
GEN_MODEL = "models/gemini-2.0-flash-exp-image-generation"
...
# --- THE VOGUE SHIELD V3.1 (Aggressive Euphemisms) ---
def vogue_sanitize(text):
    if not text: return ""
    euphemisms = {
        # REPARTO "ORRORE DEI FILTRI" (Bikini, ecc.) RIDEFINITO TECNICAMENTE
        r"\b(bikini|costume|swimwear|due pezzi|swimsuit)\b": "layered architectural technical silk two-piece ensemble",
        r"\b(bra|reggiseno|soutien|top|triangolo|fascia)\b": "structured sculpted bodice in technical fabric",
        r"\b(slip|mutanda|panties|calcinha|cueca|bottom|brasiliana)\b": "high-cut minimalist silhouette technical bottom",
        r"\b(thong|perizoma|fio dental|tanga)\b": "ultra-minimalist technical couture structure",

        # Eufemismi standard (già presenti)
        ...
        r"\b(cleavage|scollatura|decote)\b": "glamorous decolletage framing",
        r"\b(breast|seno|seios|peito|tette)\b": "feminine torso silhouette (cup D volume)",
        r"\b(butt|booty|culo|bumbum|rabo|sedere)\b": "lower body curves silhouette",
        r"\b(see-through|trasparente|transparente|sheer)\b": "sheer translucent luxury technical fabric",
        r"\b(sexy|hot|quente|seducente|provocante)\b": "alluring, sophisticated and high-editorial aesthetic",
        r"\b(peli|pelo|hair on body|peloso)\b": "completely smooth hairless polished skin"
    }
    sanitized = text.lower()
    for pattern, replacement in euphemisms.items():
        sanitized = re.sub(pattern, replacement, sanitized)
    # Rimuove doppie spaziature create dalle sostituzioni
