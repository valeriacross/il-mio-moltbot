...
FILTERS = {
    ...
    "cinematic_highangle": {
        ...
        "cat": "stylistic",
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Apply a cinematic high-angle photographic filter: reframe the shot from an overhead perspective, "
            "the subject looks up toward the camera with intense expressive eyes and sharp focus. "
            "Keep the subject's identity, face, clothing and physical features identical to the reference. "
            "Shallow depth of field with softly blurred background, dramatic soft lighting with subtle shadows, "
            "moody color grading, high contrast, ultra-realistic skin tones, "
    ...
    "dramatic": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Apply a dramatic low-angle photographic filter: reframe the shot from an extreme low-to-high angle, "
            "sharply focusing on the foreground subject. Keep the subject's identity, face, clothing and pose identical to the reference. "
    ...
    "stained_glass": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Apply this filter to the image: transform the subject entirely into stained glass and translucent crystal, "
            "preserving their silhouette, pose and proportions. "
            "The whole body becomes a mosaic of milky white and pale blue glass tiles, held together by an intricate polished silver wire frame. "
            "Eyes and hair are rendered in sculpted glass matching their original color. "
    ...
    "graffiti": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Create one ultra-realistic image showing the exact subject from the reference photo spray-painting a full-body "
            "self-portrait graffiti on an urban brick wall. The subject is standing or half-crouched, mid-spray, "
            "while the entire head-to-toe mural is clearly visible on a single continuous brick wall with realistic texture. "
            "The graffiti mural reproduces the subject's likeness accurately. "
            "The graffiti looks fresh with natural overspray and paint drips, spray mist is visible, and several unbranded "
            "spray cans lie on the ground. The subject's face, body, outfit, and identity remain exactly the same as the reference, "
    ...
    "giantess": {
        ...
        "prompt": (
            ...
            "SCENARIO: the subject from the reference photo walks carefully down the center of Broadway in New York City, "
            "their footwear spanning several city blocks while their head towers above the spire of the Chrysler Building. "
            "The subject is an actual human of colossal proportions (hundreds of meters tall). "
    ...
    "action_figure": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Create a 1/7 scale commercial figure of the exact subject from the reference photo, in a realistic style set in a real environment. "
    ...
    "new_pose": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Create a new image using the exact same scene, environment, outfit and identity as the reference photo, "
    ...
    "triple_set": {
        ...
        "prompt": (
            ...
            "Change pose and expression in each image, but maintain the subject's exact identity, outfit, and environment. "
            "Maintain original aspect ratio and exact proportions. Seed: random for each."
        )
    },
    "triptych": {
        ...
        "prompt": (
            "Using the uploaded photo as the sole reference, preserve exactly the subject shown — same face, same identity, same outfit, same body, same pose. Do not substitute, replace or reinvent the subject under any circumstance. "
            "Create a 3:1 collage of three versions of the exact subject from the reference photo, "
            "with completely different poses and facial expressions. Seed: random. "
    ...
    "remix_mosaic": {
        ...
        "cat": "collage",
        "use_master": False,
        "prompt": None,  # gestito direttamente in _run_generation
    },
}
...
def _send_confirmation(chat_id, uid, fname, reply_to=None):
    ...
    # remix_mosaic (e altri filtri dinamici) hanno prompt=None — sostituisci con placeholder
    if filter_prompt is None:
        filter_prompt = "🎲 Prompt generato dinamicamente a runtime (8 filtri random dal pool)."
...
# --- REMIX MOSAIC ---
REMIX_POOL = [
    "cinematic_highangle", "dramatic", "glossy", "iridescent",
    "rainbow_neon", "galaxy", "neon_hdr", "ghost_temporal",
    "3d_synthetic", "graffiti", "cloud_sculpture",
]

def assemble_mosaic(original_bytes, tile_results):
    """
    Assembla una griglia 3x3 con Pillow.
    tile_results: lista di 8 bytes (o None se falliti), in ordine top-left → bottom-right escludendo centro.
    Posizioni: 0,1,2 / 3,[centro],4 / 5,6,7

    La dimensione tile viene ricavata dalla prima tile generata da Gemini (non dall'originale),
    così tutte le celle — incluso l'originale al centro — hanno dimensioni uniformi.
    """
    # Trova la prima tile valida per ricavare le dimensioni di riferimento
    tw, th = None, None
    for tb in tile_results:
        if tb:
            try:
                sample = Image.open(io.BytesIO(tb)).convert("RGB")
                tw, th = sample.size
                logger.info(f"   🖼️ Dimensione tile di riferimento: {tw}×{th}px")
                break
            except Exception:
                continue

    # Fallback: se nessuna tile è valida usa l'originale
    if tw is None:
        orig_ref = Image.open(io.BytesIO(original_bytes)).convert("RGB")
        tw, th = orig_ref.size
        logger.warning(f"   ⚠️ Nessuna tile valida — uso dimensioni originale: {tw}×{th}px")

    grid_w = tw * 3
    grid_h = th * 3
    grid = Image.new("RGB", (grid_w, grid_h), (20, 20, 20))

    # Posizioni griglia 3x3, centro = indice (1,1)
    positions = [
        (0,0),(1,0),(2,0),
        (0,1),      (2,1),
        (0,2),(1,2),(2,2),
    ...
    for i, (col, row) in enumerate(positions):
        tile_bytes = tile_results[i] if i < len(tile_results) else None
        if tile_bytes:
            try:
                tile_img = Image.open(io.BytesIO(tile_bytes)).convert("RGB")
                tile_img = tile_img.resize((tw, th), Image.LANCZOS)
                grid.paste(tile_img, (col * tw, row * th))
            except Exception as e:
                logger.warning(f"   ⚠️ Tile {i} non assemblabile: {e}")

    # Incolla originale al centro — ridimensionato alla stessa dimensione delle tile
    orig = Image.open(io.BytesIO(original_bytes)).convert("RGB")
    orig = orig.resize((tw, th), Image.LANCZOS)
    grid.paste(orig, (tw, th))
...
def _run_generation(chat_id, uid, username, data):
    ...
    # ── REMIX MOSAIC ─────────────────────────────────────────────────────────
    if fkey == "remix_mosaic":
        selected = random.sample(REMIX_POOL, 8)
        # Pool di riserva: filtri non selezionati, da usare come fallback su safety block
        reserve_pool = [f for f in REMIX_POOL if f not in selected]
        logger.info(f"   🎲 Remix Mosaic — filtri: {selected} | riserva: {reserve_pool}")

        # Messaggio di stato progressivo — verrà modificato man mano che le tile completano
        status_msg = bot.send_message(chat_id,
            f"🎲 <b>Remix Mosaic</b> — avvio generazione...\n"
            f"⬜⬜⬜⬜⬜⬜⬜⬜  0/8\n"
            f"⏳ Attendi fino a 90 secondi.")

        tile_results = [None] * 8
        completed_count = 0
        status_lock = threading.Lock()

        def update_status(done, total=8, note=""):
            """Aggiorna il messaggio di stato con barra di avanzamento."""
            filled = "🟩" * done
            empty  = "⬜" * (total - done)
            text = (
                f"🎲 <b>Remix Mosaic</b> — generazione in corso...\n"
            ...
            try:
                ...
                pass  # edit fallita (flood limit o messaggio invariato) — non bloccante

        def generate_tile(idx, fk):
            nonlocal completed_count
            MAX_RETRIES = 2  # fino a 3 tentativi totali per tile
            tried_filters = [fk]

            for attempt in range(MAX_RETRIES + 1):
                current_fk = tried_filters[-1]
                fp = resolve_prompt(current_fk)
                prompt = EDITORIAL_WRAPPER + fp
                try:
                    img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
                    response = client.models.generate_content(
                        model=MODEL_TEXT_ID,  # Flash — quota molto più alta di Pro
                    ...
                    if response.candidates and response.candidates[0].finish_reason == "STOP":
                        for part in response.candidates[0].content.parts:
                            if part.inline_data:
                                tile_results[idx] = part.inline_data.data
                                logger.info(f"   ✅ Tile {idx+1}/8 ({current_fk}) OK (tentativo {attempt+1})")
                                with status_lock:
                                    completed_count += 1
                                    update_status(completed_count)
                                return
                        logger.warning(f"   ⚠️ Tile {idx+1}/8 ({current_fk}) — nessuna immagine nella risposta")
                    else:
                        reason = response.candidates[0].finish_reason if response.candidates else "NO_CANDIDATE"
                        logger.warning(f"   🛡️ Tile {idx+1}/8 ({current_fk}) bloccata: {reason} (tentativo {attempt+1})")
                        # Su safety block: prova un filtro alternativo dal pool di riserva
                        if attempt < MAX_RETRIES and reserve_pool:
                            unused_reserve = [r for r in reserve_pool if r not in tried_filters]
                            if unused_reserve:
                                new_fk = random.choice(unused_reserve)
                                tried_filters.append(new_fk)
                                logger.info(f"   🔄 Tile {idx+1}/8 — retry con filtro alternativo: {new_fk}")
                                continue
                except Exception as e:
                    logger.error(f"   ❌ Tile {idx+1}/8 ({current_fk}) errore (tentativo {attempt+1}): {e}")

            # Tutti i tentativi esauriti — tile fallita, aggiorna contatore comunque
            logger.warning(f"   ❌ Tile {idx+1}/8 esaurita dopo {MAX_RETRIES+1} tentativi")
            with status_lock:
                completed_count += 1
                update_status(completed_count, note=f"tile {idx+1} non generabile, saltata")

        # Executor dedicato e separato — evita deadlock con l'executor globale.
        # _run_generation gira già dentro un worker dell'executor globale (max_workers=4);
        # usarlo di nuovo per le 8 tile causerebbe stallo perché i worker liberi
        # si esauriscono e nessuno può completare per sbloccare gli altri.
        from concurrent.futures import ThreadPoolExecutor as _TPE
        with _TPE(max_workers=8) as tile_executor:
            futures = [tile_executor.submit(generate_tile, i, fk) for i, fk in enumerate(selected)]
            for f in futures:
                try:
                    f.result(timeout=90)
                except Exception as e:
                    logger.warning(f"   ⚠️ Tile timeout: {e}")

        elapsed = round(time.time() - t_start, 1)
        success_count = sum(1 for t in tile_results if t is not None)
        logger.info(f"   🎲 Remix Mosaic: {success_count}/8 tile OK in {elapsed}s")

        if success_count == 0:
            try:
                bot.edit_message_text(
                    "❌ <b>Remix Mosaic</b> — nessuna tile generata. Riprova.",
                    chat_id, status_msg.message_id, parse_mode="HTML")
            except Exception:
                bot.send_message(chat_id, "❌ Nessuna tile generata. Riprova.")
            return

        # Aggiorna status: assemblaggio in corso
        try:
            bot.edit_message_text(
                f"🎲 <b>Remix Mosaic</b> — assemblaggio griglia...\n"
                f"🟩🟩🟩🟩🟩🟩🟩🟩  {success_count}/8 ✅\n"
                f"⚙️ Composizione in corso...",
                chat_id, status_msg.message_id, parse_mode="HTML")
        except Exception:
            pass

        try:
            mosaic_bytes = assemble_mosaic(img_bytes, tile_results)
            logger.info(f"   🖼️ Mosaico assemblato: {len(mosaic_bytes)} bytes")
        except Exception as e:
            logger.error(f"   ❌ Crash assemble_mosaic: {e}", exc_info=True)
            try:
                bot.edit_message_text(
                    f"❌ <b>Remix Mosaic</b> — errore nell'assemblaggio.\n<code>{html.escape(str(e))}</code>",
                    chat_id, status_msg.message_id, parse_mode="HTML")
            except Exception:
                bot.send_message(chat_id, f"❌ Errore assemblaggio mosaico:\n<code>{html.escape(str(e))}</code>")
            return
        ...
        try:
            bot.send_document(
                ...
                visible_file_name="fx_remix_mosaic.jpg",
                caption=f"✅ Remix Mosaic — {success_count}/8 tile — {elapsed}s"
            )
            # Aggiorna status a completato
            try:
                bot.edit_message_text(
                    f"✅ <b>Remix Mosaic completato</b> — {success_count}/8 tile — {elapsed}s",
                    chat_id, status_msg.message_id, parse_mode="HTML")
            except Exception:
                pass
            bot.send_message(chat_id, "Cosa vuoi fare adesso?", reply_markup=post_markup)
        except Exception as e:
            logger.error(f"   ❌ Errore send_document mosaico: {e}", exc_info=True)
            try:
                bot.edit_message_text(
                    f"❌ <b>Remix Mosaic</b> — mosaico generato ma errore nell'invio.\n<code>{html.escape(str(e))}</code>",
                    chat_id, status_msg.message_id, parse_mode="HTML")
            except Exception:
                bot.send_message(chat_id, f"❌ Errore invio mosaico:\n<code>{html.escape(str(e))}</code>")
        return
    # ─────────────────────────────────────────────────────────────────────────
