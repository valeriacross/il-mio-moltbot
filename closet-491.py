...
def handle_input(m):
    ...
    if m.reply_to_message and m.reply_to_message.message_id in generated_images:
        ...
        preview = (
            f"↩️ <b>Modifica su scatto precedente</b>\n\n"
            f"📝 <b>Istruzione:</b> {html.escape(new_instruction)}\n"
            f"📐 <b>Formato:</b> {settings['ratio']}\n"
            f"🔢 <b>Quantità:</b> {settings['count']} foto"
        )
        markup = InlineKeyboardMarkup()
        markup.row(
        ...
        bot.reply_to(m, f"📋 <b>Riepilogo modifica:</b>\n\n{preview}\n\nProcedere?", reply_markup=markup)
        return
    ...
    # Anteprima testo semplice
    has_photo = "✅ Sì" if img_data else "❌ No"
    preview = (
        f"👗 <b>Outfit allegato:</b> {has_photo}\n"
        f"📝 <b>Istruzione:</b> {html.escape(user_instruction)}\n"
        f"📐 <b>Formato:</b> {settings['ratio']}\n"
        f"🔢 <b>Quantità:</b> {settings['count']} foto"
    )
    ...
    bot.reply_to(m, f"📋 <b>Riepilogo:</b>\n\n{preview}\n\nProcedere?", reply_markup=markup)
