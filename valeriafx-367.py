...
def handle_mosaic_build(call):
    ...
    if uid not in mosaic_collecting:
        ...
        return
    _finalize_mosaic(uid, call.message.chat.id)
...
def _finalize_mosaic(uid, chat_id):
    ...
    if n == 4:
        ...
        if needed:
            ...
            bot.send_message(chat_id,
                f"❌ Hai inviato <b>{n} foto</b> — massimo 9 (3×3).\n"
                f"Usa /mosaic per ricominciare.")
        mosaic_collecting[uid] = {'photos': photos, 'timer': None, 'aspect': data.get('aspect')}
        return

    aspect_label = data.get('aspect', '?')
    bot.send_message(chat_id, f"🖼️ Assemblando mosaico {cols}×{rows} ({aspect_label})...")
    ...
    def build():
        try:
            ...
            # Ridimensiona tutte alla dimensione della più piccola — proporzionale con letterbox
            ...
            tiles = [fit_tile(img, min_w, min_h) for img in images]
            ...
            bot.send_document(
                ...
                caption=f"✅ Mosaico {cols}×{rows} ({aspect_label}) — {n} foto"
            )
    ...
    if uid in mosaic_collecting:
        ...
        if n in (4, 9):
            ...
            markup.add(types.InlineKeyboardButton(
                f"✅ Assembla {cols}×{cols} ({aspect_label})",
                callback_data=f"mosaic_build_{uid}"
            ))
            bot.reply_to(m,
                f"📷 <b>{n} foto ricevute</b> — pronto per il mosaico {cols}×{cols}!\n"
                f"Formato rilevato: <b>{aspect_label}</b>",
                reply_markup=markup
            )
        elif n > 9:
            bot.reply_to(m, f"⚠️ Hai già {n} foto — massimo 9 (3×3). Usa /mosaic per ricominciare.")
        else:
            needed_next = 4 if n < 4 else 9
            bot.reply_to(m, f"📷 Foto {n} ricevuta. Ancora <b>{needed_next - n}</b> per il prossimo formato ({aspect_label}).")
    ...
    try:
        ...
        for part in candidate.content.parts:
            if part.inline_data:
                ...
                if caption:
                    bot.send_message(chat_id,
                        f"{caption}\nhttps://t.me/valeriacross_gallery")
                bot.send_message(chat_id,
