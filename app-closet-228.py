# ---------------------------------------------------------
# VERSION: V2.28 (Smart Context: Temu vs Reality)
# TIMESTAMP: 2026-02-14 23:10 WET
# CHANGELOG:
# - New Vision Logic: Detects if background is "Studio/Plain" or "Specific".
# - Smart Environment:
#    * If Background = Specific (Snow, Street) -> REPLICATE IT.
#    * If Background = Plain (Temu/Catalog) -> INVENT LUXURY CONTEXT.
# - Core: V2.27 Base (Pro Engine, Safety Bypass, Discrete Files).
# ---------------------------------------------------------
...
def show_settings(m):
    bot.send_message(m.chat.id, "<b>👗 Valeria Closet V2.28 (Smart Context)</b>\nScegli il formato e il numero di scatti:", 
...
def analyze_outfit_vision(img_bytes):
    try:
        # NUOVO PROMPT VISIONE: Chiede di analizzare ANCHE lo sfondo
        prompt = """
...
def generate_closet_task(img_bytes, ar_scelto, user_notes, vision_desc, edit_mode=False):
    try:
        ...
        Regola OUTFIT & AMBIENTAZIONE (Smart Context)
        Analisi Immagine: {safe_outfit}
        
        LOGICA AMBIENTAZIONE:
        1. SE l'analisi rileva un LUOGO SPECIFICO (es. Neve, Strada, Spiaggia, Bosco): MANTIENI quell'ambientazione. Ricrea l'atmosfera, la luce e lo sfondo descritti nell'immagine di riferimento.
        2. SE l'analisi dice "Studio", "Plain" o "Neutral": ALLORA INVENTA un'ambientazione di lusso coerente con l'abito (es. Lingerie->Camera da letto Luxury, Bikini->Spiaggia Esotica, Abito->Gala, Casual->Urbana).
        ...
        female face, woman, girl, young, unrealistic skin, long feminine hair, body hair, chest hair, breast hair, peli sul seno, static pose, mannequin pose, watermark invasivo.
        """
...
def handle_reply_edit(m):
    ...
    if file_id:
        bot.reply_to(m, "🔄 Elaborazione modifica (V2.28)...")
...
@app.route('/')
def h(): return "Bot Online - V2.28 (Smart Context)"
